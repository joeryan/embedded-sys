...
void dump_data(unsigned long i, unsigned long last, unsigned now); // data dump
...
void toggle_LED(void) { unsigned long Led;// subroutine to toggle LED and dump register
...
void dump_data(unsigned long i, unsigned long last, unsigned now){ 
  if(i<50){
...
int main(void){  unsigned long i, last, now, sw1_pf0, sw2_pf4, Led;
  ...
  while(1){
		sw1_pf0 = GPIO_PORTF_DATA_R & 0x01;
		sw2_pf4 = GPIO_PORTF_DATA_R & 0x10;
		while(sw1_pf0 != 0x01 || sw2_pf4 != 0x10) {
			toggle_LED();
	    now = NVIC_ST_CURRENT_R;
			dump_data(i, last, now);
			last = now;
			i++;
			Delay();
			sw1_pf0 = GPIO_PORTF_DATA_R & 0x01;
