...
// as part of Lab 11 implement this function
  for (i = 0; i < strlen((char *)buffer);i++) {
		UART_OutChar((unsigned char) buffer[i]);
	}
}
...
void UART_ConvertUDec(unsigned long n){ unsigned long digit;
// as part of Lab 11 implement this function
	if (n > 9999) {
		strcpy((char *) String, "**** ");
	} else if (n == 0) {
		...
		strcpy((char *)String, ""); // make String empty
		for (digit = 4 - count; digit > 0; digit --) {
			strcat((char *) String, " ");
		} // pad with leading spaces
		for(; count > 0; count --) {
			// place each digit into string in correct order
			strncat((char *) String, (const char *) &(DIGITS[digits[count-1]]), 1 );
		}
		strcat( (char *) String, " ");
...
void UART_ConvertDistance(unsigned long n){ unsigned long digit;
// as part of Lab 11 implement this function
  if (n > 9999) {
		...
		strcpy( (char *) String, "");
		strncat( (char *) String, &(DIGITS[digits[count-1]]), 1);
		strcat( (char *) String, ".");
		count --;
		for(; count > 0; count --) {
			strncat( (char *) String, &(DIGITS[digits[count-1]]), 1);
